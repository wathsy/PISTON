...
public:
  ...
  unsigned long long side, size;                                  // variable needed to determine the neighborhood cubes
  ...
  halo_merge(float min_linkLength, float max_linkLength, int k = 2, std::string filename="", std::string format=".cosmo", int n = 1, int np=1, float rL=-1) : halo(filename, format, n, np, rL)
  {
    if(numOfParticles!=0)
    {
      ...
      std::cout << std::endl << "side " << side << " cubeSize " << size << " ite " << ((size-1)/2) << std::endl << std::endl;
...
//      for(unsigned long long l=startOfChunks[i]; l<startOfChunks[i]+chunkSize; l++)
  ...
  void globalStep()
  {
    ...
    while(true)
    {
      ...
      std::cout << "----tmpNxt "; thrust::copy(tmpNxt.begin(), tmpNxt.begin()+2, std::ostream_iterator<long long>(std::cout, " ")); std::cout << " ";
      ...
      std::cout << "----edgesSrc "; thrust::copy(edgesSrc.begin(), edgesSrc.begin()+2, std::ostream_iterator<long long>(std::cout, " ")); std::cout << " ";
